...
def create_execution(db: Session, execution_in: execution_schema.ExecutionCreate,
                     user: models.User) -> models.PlanItemExecution:
    ...
    if plan_item.version.plan.created_by != user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Нет прав для добавления отчета к этой позиции")

    # Проверяем статус плана
    if plan_item.version.status != models.PlanStatus.APPROVED:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="Отчеты можно добавлять только к утвержденным планам")

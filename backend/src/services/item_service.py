...
def update_item(db: Session, item_id: int, item_in: plan_schema.PlanItemUpdate,
    ...
    if version.status != models.PlanStatus.DRAFT:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Редактирование запрещено, версия не в статусе 'Черновик'.")
    ...
    if 'trucode' in update_data:
        ...
        if enstru_item:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                detail=f"Код ЕНС ТРУ '{update_data['trucode']}' не найден.")
...
def delete_item(db: Session, item_id: int, user: models.User) -> bool:
    ...
    if version.status != models.PlanStatus.DRAFT:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Удаление запрещено, версия не в статусе 'Черновик'.")
...
def revert_item(db: Session, item_id: int, user: models.User) -> models.PlanItemVersion:
    ...
    if db_item.source_version_id != version.id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="Эта позиция не была изменена в текущей версии.")

...
def process_kenml_import(db: Session, file: UploadFile):
    ...
    for key, val in goods.items():
        ...
        final_rows.append(
            {'cat': cat, 'name': name, 'code_agsk': code, 'unit': unit, 'vol': vol, 'price': price, 'total': total})

    for key, val in works_services.items():
        cat, name, code = key
        total = val['total']
        final_rows.append(
            {'cat': cat, 'name': name, 'code_agsk': code, 'unit': '', 'vol': 1.0, 'price': total, 'total': total})
    ...
    if all_found_enstru_codes:
        ...
        for code, name in db.query(models.Enstru.code, models.Enstru.name_rus).filter(
                models.Enstru.code.in_(all_found_enstru_codes)).all():

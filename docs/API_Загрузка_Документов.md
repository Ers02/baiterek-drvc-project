...
| `external_id` | строка | ❌ | ID документа в вашей системе (для сверки) |
...
| Статус | Описание |
|--------|---------|
| `NEW` | Документ загружен, ожидает назначения аналитику |
| `PARSED` | Файл распарсен, позиции извлечены |
| `COMPLETED` | Анализ завершён, результат сформирован |
